...
$query = "SELECT COUNT(id)
          FROM posts
          WHERE DATE_FORMAT(create_at, '%Y-%m') ='$create_at'";
...
$start = ($page -1) * 5;
//最新記事から5記事ごと表示
$query = "SELECT * , LEFT(body,250)as content FROM posts WHERE DATE_FORMAT(create_at, '%Y-%m') ='$create_at' ORDER BY id DESC LIMIT {$start},5";
?>
...
<?php }
  ...
  <br>
  <h4><label>アーカイブはありません。</label></h4>
<?php } ?>
<!-- ページング -->
<p>
  <?php if ($page > 1) : ?>
    <a href="arcive.php?page=<?php echo ($page - 1); ?>&create_at=<?php if(!empty($create_at)){ echo $create_at; } ?>">前のページ </a>
  <?php endif; ?>

  <?php if ($page < $maxPage) : ?>
    <a href="arcive.php?page=<?php echo ($page + 1); ?>&create_at=<?php if(!empty($create_at)){ echo $create_at; } ?>"> 次のページ</a>
  <?php endif; ?>
</p>
</div>
<!-- サイドメニュー -->
