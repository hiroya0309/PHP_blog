...
$page = max($page, 1);
//最終ページを決める
$query = "SELECT COUNT(id) FROM posts WHERE title LIKE '%$kennsaku%'" ;
...
$start = ($page -1) * 5;
//最新記事から5記事ごと表示
$query = "SELECT * , LEFT(body,250)as content FROM posts WHERE title LIKE '%$kennsaku%' ORDER BY id DESC LIMIT {$start},5";
?>
...
<?php }
  ...
  <br>
  <h4><label>検索結果はありません。</label></h4>
<?php } ?>
<!-- ページング -->
<p>
  <?php if ($page > 1) : ?>
    <a href="home-kennsaku.php?page=<?php echo ($page - 1); ?>&kennsaku=<?php if(!empty($kennsaku)){ echo $kennsaku; } ?>">前のページ </a>
  <?php endif; ?>

  <?php if ($page < $maxPage) : ?>
    <a href="home-kennsaku.php?page=<?php echo ($page + 1); ?>&kennsaku=<?php if(!empty($kennsaku)){ echo $kennsaku; } ?>"> 次のページ</a>
  <?php endif; ?>
</p>
</div>
<!-- サイドメニュー -->
